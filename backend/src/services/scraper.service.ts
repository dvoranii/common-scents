...
class ScraperService {
  ...
  async scrapeFragranceReviews(
    ...
    try {
      ...
      const scraperUrl = `https://api.scraperapi.com/?api_key=${this.apiKey}&url=${encodeURIComponent(url + "#all-reviews")}&render=true`;
      ...
      $(".fragrance-review-box").each((i, element) => {
        ...
        const author = $review.find('[itemprop="author"] [itemprop="name"]').attr("content") || "";
        const date = $review.find('[itemprop="datePublished"]').attr("content") || "";
        const text = $review.find('[itemprop="reviewBody"]').text().trim() || "";
